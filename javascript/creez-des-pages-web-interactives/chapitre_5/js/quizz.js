...
for (i=0;i<questions.length;i++){
    ...
    numeroQuestion[i] = document.createElement("strong");
    numeroQuestion[i].textContent = "Question "+(i+1)+": ";
    textQuestion[i] = document.createTextNode(questions[i].enonce);
    bouton [i] = document.createElement("div");
    bouton[i].innerHTML = "<button>reponse</button>";
   
    
    question[i].appendChild(numeroQuestion[i]);
    question[i].appendChild(textQuestion[i]);
    question[i].appendChild(bouton[i]);
    document.getElementById("contenu").appendChild(question[i]);
    
     bouton[i].addEventListener("click", function (e) {
        laReponse[i] = document.createTextNode(questions[i].reponse);
        bouton[i].innerHTML = "";
        bouton[i].appendChild(laReponse[i]);
    })
}
